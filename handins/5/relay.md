...
It does not influence the decision making process of the parties either even with the byzantine sender, the parties would end up with multiple messages _m_ in the _Relayed<sub>i</sub>(bid, m) = T_ and thus outputing  _(bid, P<sub>i</sub>, NoMsg)_ on _Cast<sub>j</sub>_. The only thing the proposed change does is, that it verifies the integrity og the _bid_ - that the broadcast identifier is unique for each message and there are no two different messages, _m<sub>i</sub> != m<sub>j</sub>_ such as _bid<sub>i</sub> = bid<sub>j</sub>_.
